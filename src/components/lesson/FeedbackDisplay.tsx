...
export function FeedbackContent({ isCorrect, exercise, userAnswer }: FeedbackContentProps) {
  ...
  return (
    ...
    >
      ...
      {distractor && (
        <div className="border-l-4 border-red-500 dark:border-red-400 bg-white dark:bg-surface-900 px-3 py-2 rounded">
          ...
          <MarkdownContent className="text-ink dark:text-surface-100">{distractor}</MarkdownContent>
        </div>
      )}

      {exercise.explanation && (
        <div className="border-l-4 border-ink dark:border-surface-400 bg-white dark:bg-surface-900 px-3 py-2 rounded">
          <p className="font-mono text-[10px] font-black text-ink dark:text-surface-100 uppercase tracking-widest mb-1">
            // {isCorrect ? 'Erklärung' : 'Lösungsweg'}
          </p>
          <MarkdownContent className="text-ink dark:text-surface-100">{exercise.explanation}</MarkdownContent>
        </div>
      )}
    </div>
  )
