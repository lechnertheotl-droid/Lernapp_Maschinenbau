export type FeedbackTone = 'encouraging' | 'celebratory' | 'corrective'
...
export function generateFeedback({
  ...
  if (isCorrect) {
    ...
    const tone: FeedbackTone = streakCount >= 5 ? 'celebratory' : 'encouraging'
    return { message, tone }
  }
  return { message: randomFrom(INCORRECT_MESSAGES), tone: 'corrective' }
}

export function getEncouragementMessage(streakCount: number): string | null {
